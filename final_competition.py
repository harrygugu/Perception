...
class KeyboardPlayerPyGame(Player):
    def __init__(self):
        ...
        self.save_dir = "./data/exploration_data_maze_1/images_subsample/"
    ...
    def act(self):
        ...
        if self.automatic_mode and not self.paused:
            ...
            if self.automatic_mode and self.paused and not self.wall_detected():
                if self.paused:
                    print("\nWall cleared! Resuming automatic mode.")
                self.paused = False
        ...
        if self.movement_state == Action.FORWARD:
            ...
            self.last_action = self.movement_state
            return self.movement_state
        elif self.movement_state == Action.LEFT:
            self.last_action = self.movement_state
            return self.movement_state
        elif self.movement_state == Action.RIGHT:
            self.last_action = self.movement_state
            return self.movement_state
        ...
        if self._state:
            ...
            # If in navigation stage
                ...
                if self.goal_id is None:
                    ...
                    # Ask user the path to go: decision = user keyboard inpui 1-4
                    while True:
